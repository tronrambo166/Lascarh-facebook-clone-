...
include "like_comment_share.php";
	...
	//-----------------------------------------------
	    $main_id=$_COOKIE['user_id']; $get_com = "select * from users where ID='$main_id'";
		$run_com = $db->select($get_com);
		$commentor =$run_com->fetch_assoc();

		
	
	
		$user_id = $_GET['others_profile_id'];
...
<?php $my_profile=$_COOKIE['user_id']; $get_user="select * from users where ID='$my_profile'"; $my_row=$db->select($get_user)->fetch_assoc(); ?>
			 
			<img class="rounded-circle mr-2" width="28px" height="26px" src="u_images/<?php echo $my_row['Photo']; ?>" alt="" /><?php echo $my_row['Fname']; ?></a></li>
...
<div class="row">
	<div class="col-sm-5 ml-5 mt-4 ">
	
			
			<div>
				<div><img height="300;" style="border-radius: 77px" id='cover-img' class=' w-100' 
				src='u_images/<?php if($row['Cover']=='') echo 'cover.png';  else echo $row['Cover']; ?>' alt='cover'></div>
				
				
			</div>
			
			<div id='profile-img' class=" row w-50 mx-auto">
				<img src='u_images/<?php if($row['Photo']=='') echo 'profile-placeholder.jpg';  else echo $row['Photo']; ?>' 
				alt='Profile' class='rounded-circle mt-2 mb-1  d-inline' width='175px' height='175px'>
				
				<div class="d-inline-block p_name ml-2" style="height:100px font-family:cursive">
				<h4 style=" font-family:cursive;position: absolute;top: 100px" class="w-100 d-block text-white font-weight-bold"><?php echo $user_name;?></h4></div>
			...
			<h2 class=" h4 d-inline"><strong>(About) </strong></h2><strong><?php echo $user_name;?></strong> 
			<a href="sent_messages.php?to_id=<?php echo $user_id; ?>" class=" rounded btn btn-primary float-right"><i class="fa fa-envelope"></i> Message</a>

			
			<div class="my-3 border border-info"></div>
...
<?php }?>
		...
		<?php  } ?>
		 ...
		 <form action="" method="POST">
				...
				<input type="text" hidden name="com_name" value="<?php echo $commentor['Fname'].' '.$commentor['Lname']; ?>" />
				<input type="number" hidden name="com_id" value="<?php echo $main_id; ?>" />
				<input type="text" hidden name="user_image" value="<?php echo $commentor['Photo']; ?>" />
